...
protected:
    ...
    // Get関数
    int &GetNumVertex()                                  { return m_nNumVertex; }                    // 頂点数の情報
    int &GetNumIndex()                                   { return m_nNumIndex; }                     // インデックス数の情報
    int &GetNumPolygon()                                 { return m_nNumPolygon; }                   // ポリゴン数の情報
    CYLINDER &GetCylinderNum(const MESH_TYPE &typeNum)   { return m_Mesh3D[typeNum].CylinderNum; }   // 何角形かの情報
    MESH_TYPE &GetMeshNum()                              { return m_MeshNum; }                       // メッシュの番号
    D3DXVECTOR2 &GetOneSize()                            { return m_OneSize; }                       // サイズ
    INT_VERTEX_2D &GetPieceNum()                         { return m_PieceNum; }                      // ブロックの数
    LPDIRECT3DINDEXBUFFER9 GetIdxBuff()                  { return m_pIdxBuff; }                      // バッファ番号の情報
    D3DXMATRIX GetMtxWorld()                             { return m_mtxWorld; }                      // マトリクス情報
