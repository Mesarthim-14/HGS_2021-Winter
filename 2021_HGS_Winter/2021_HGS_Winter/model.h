...
protected:
    ...
    void SetTexPattern(const int &TexPattern)    { m_nTexPattern = TexPattern; } // テクスチャのパターン
    void SetLife(const int &nLife)               { m_nLife = nLife; }            // ライフの設定
    void SetScale(const D3DXVECTOR3 &scale)      { m_scale = scale; }            // 拡大率

    D3DXVECTOR3 &GetMove()       { return m_move; }          // 移動量の情報
    D3DXVECTOR3 GetSize()const   { return m_size; }          // サイズの設定
    int GetTexPattern()const     { return m_nTexPattern; }   // テクスチャ
    int GetLife()const           { return m_nLife; }         // ライフの設定
